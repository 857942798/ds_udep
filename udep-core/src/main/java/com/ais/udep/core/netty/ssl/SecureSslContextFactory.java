package com.ais.udep.core.netty.ssl;
...
public final class SecureSslContextFactory {
    ...
    private static final String KEY_STOREPASS="udepNetty123";
    ...
    public static SSLContext getServerContext(String pkPath){
        ...
        try{
            ...
            if(pkPath!=null){
                ...
                //加载服务端证书
                Resource[] res= resolver.getResources(pkPath);
                //加载服务端的KeyStore  ；udepSslKey是生成仓库时设置的密码，用于检查密钥库完整性的密码
                ks.load(res[0].getInputStream(), KEY_STOREPASS.toCharArray());

                kmf = KeyManagerFactory.getInstance("SunX509");
                //初始化密钥管理器
                kmf.init(ks, KEY_STOREPASS.toCharArray());
            }
            //获取安全套接字协议（TLS协议）的对象
    ...
    public static SSLContext getClientContext(String caPath){
        ...
        try{
            ...
            Resource[] res= resolver.getResources(caPath);

            if (caPath != null) {
                //密钥库KeyStore
                KeyStore tks = KeyStore.getInstance("JKS");
                //加载客户端证书
                tks.load(res[0].getInputStream(), KEY_STOREPASS.toCharArray());
